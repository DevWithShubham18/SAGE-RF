...
function RFCore() {
  ...
  return (
    <group ref={group}>
      ...
      <pointLight color="#22d3ee" intensity={5} distance={8} />
...
function App() {
  ...
  const spectrumData = useMemo(() => {
    ...
    const center = spectrum.peak_frequency_hz ?? 0;
    const bandwidth = spectrum.occupied_bandwidth_hz ?? 1000;

    return Array.from({ length: 80 }, (_, index) => {
      const progress = index / 79;
      const frequency =
        center - bandwidth * 1.5 + progress * bandwidth * 3;

      const distance = Math.abs(frequency - center);
      const peak = Math.max(
        0,
        1 - distance / Math.max(bandwidth, 1)
      );

      const noise = 0.08 + Math.sin(index * 1.7) * 0.025;

      return {
        frequency,
        power: noise + peak * 0.88,
      };
    });
  }, [result]);
  ...
  async function analyzeSignal() {
    ...
    try {
      ...
      if (file.name.toLowerCase().endsWith(".iq")) {
        formData.append("iq_sample_rate", sampleRate);
      }
      ...
      if (!response.ok) {
        throw new Error(
          data.detail || "RF analysis failed."
        );
      }
  ...
  return (
    <div className="app-shell">
      ...
      <main>
        <section className="hero">
          <div className="hero-copy">
            ...
            <p>
              Upload raw RF recordings and transform complex
              signal data into actionable intelligence.
            </p>
        ...
        <section className="workspace">
          ...
          <div className="upload-panel">
            ...
            {file?.name.toLowerCase().endsWith(".iq") && (
          ...
          <section className="results-section">
            ...
            <div className="stats-grid">
              ...
              <StatCard
                ...
                value={result.detections?.candidate_count ?? 0}
                accent
              />
              ...
              <StatCard
                ...
                value={formatHz(
                  result.spectrum?.peak_frequency_hz
                )}
              />
            </div>
                  ...
                  <>
                    ...
                    <div className="evidence-grid">
                      ...
                      <div>
                        ...
                        <strong>
                          {Number(
                            modulation.evidence?.amplitude_cv ||
                              0
                          ).toFixed(4)}
                        </strong>
                      </div>
            ...
            <div className="glass-panel detection-panel">
              <div className="panel-header">
                <div>
                  ...
                  <h3>Detected RF Signals</h3>
                </div>

                <span className="count-pill">
                  {detections.length} CANDIDATE
              ...
              <div className="table-wrap">
                <table>
                  ...
                  <tbody>
                    {detections.map((candidate, index) => (
                      <tr key={`${candidate.center_frequency_hz}-${index}`}>
                        <td>
                        ...
                        <td className="positive">
                          {Number(candidate.snr_db).toFixed(
                            1
                          )}{" "}
                          dB
                        </td>
            ...
            <div className="glass-panel telemetry-panel">
              ...
              <div className="telemetry-grid">
                ...
                <div>
                  ...
                  <strong>
                    {Number(
                      result.metadata?.sample_count || 0
                    ).toLocaleString()}
                  </strong>
                </div>
      ...
      <footer>
        <div>SAGE-RF / SIGNAL INTELLIGENCE</div>
        <div>RF ANALYSIS ENGINE v1.0</div>
      </footer>
    </div>
  );
}
